...
object PetRepository {
    ...
    private val _petsFlow = MutableStateFlow<List<Pet>>(pets.toList())
    val petsFlow: StateFlow<List<Pet>> = _petsFlow

    fun getAllPets(): List<Pet> = pets.toList()

    fun getPetById(id: Int?): Pet? = pets.find { it.id == id }

    fun addPet(pet: Pet) {
        pets.add(0, pet)
        _petsFlow.value = pets.toList()
    }
    ...
    fun getNextId(): Int = pets.maxOfOrNull { it.id }?.plus(1) ?: 1
}
